...
$lang_change = "";
$site_lang_change = "";
$site_lang_current_icon = "";
$site_lang_current_title = "";
...
if ($is_loged_in) {
	...
	$site_languages = dle_ml_get_languages();
	$site_main_language = dle_ml_main_folder($config);
	$site_lang_change = "<li>";

	foreach ( $site_languages as $folder => $meta ) {
		...
		if( isset($meta['icon']) AND $meta['icon'] AND file_exists(ROOT_DIR . "/language/{$folder}/{$meta['icon']}") ) {
			...
			if( $folder == $site_main_language ) {
				$site_lang_current_icon = "<img src=\"language/{$folder}/{$meta['icon']}\">";
			}
		}

		if( $folder == $site_main_language ) {
			$site_lang_current_title = $title;
		}

		$site_lang_change .= $title . "</a>";
	}

	$site_lang_change .= "</li>";

	if( !$site_lang_current_title ) {
		$site_lang_current_title = $site_main_language;
	}
...
<head>
	<meta charset="utf-8">
	<title>Admin Panel - {$lang['skin_title']}</title>
	...
	{css_files}
	{$custom_css}
	<link rel="stylesheet" href="engine/skins/stylesheets/admin-modern.css">
	{js_files}
</head>
<body{$body_class}>
<script>
...
//-->
	...
	<div class="navbar navbar-inverse bg-primary-700">
		<div class="navbar-header">
			<a class="navbar-brand" href="?mod=main">Admin Panel</a>
			...
			<div class="navbar-right">	
				<ul class="nav navbar-nav">
					...
					<li class="dropdown">
							...
							<li class="dropdown">
								<a href="#" class="dropdown-toggle" data-toggle="dropdown" title="Site Main Language"><i class="fa fa-flag"></i><span class="visible-xs-inline-block position-right">Site Main Language</span></a>
								<ul class="dropdown-menu dropdown-menu-right site_lang_switcher_toggle">
									{$site_lang_change}
								</ul>
							</li>
...
<!--MAIN area-->
	...
	<div class="text-muted text-size-small text-center">Admin Panel</div>
