...
if ($_REQUEST['action'] == "save_main_lang") {
	...
	if( !$_REQUEST['lang'] OR !isset($languages[$_REQUEST['lang']]) ) {
		echo "{\"error\":\"error\"}";
		die ();
	}

	$config['main_language'] = $_REQUEST['lang'];

	if( !dle_ml_write_config_file($config) ) {
		echo "{\"error\":\"error\"}";
		die ();
	}

	set_cookie("site_language", $_REQUEST['lang'], 365);
	set_cookie("site_lang", dle_ml_code_from_folder_name($_REQUEST['lang']), 365);
