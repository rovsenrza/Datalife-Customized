...
if ($do == "cat" AND $category AND !$subaction) {
	...
	if ($config['allow_alt_url'] ) {
		...
		$rssturbo_url = $PHP_SELF . "?mod=rss&rssmode=turbo&do=cat&category=" . get_url ( $category_id );
...
if( $do == "feedback" ) {
	$canonical = dle_ml_public_url('contact', array(), $config);
} elseif( $do == "register" AND (!isset($doaction) OR !$doaction) ) {
	$canonical = dle_ml_public_url('register', array(), $config);
} elseif( $do == "lostpassword" AND !isset($_REQUEST['action']) ) {
	$canonical = dle_ml_public_url('lostpassword', array(), $config);
} elseif( $do == "lastcomments" ) {
	$lastcomments_params = array();
	if( isset($_REQUEST['userid']) AND intval($_REQUEST['userid']) ) {
		$lastcomments_params['userid'] = intval($_REQUEST['userid']);
	}
	$canonical = dle_ml_public_url('lastcomments', $lastcomments_params, $config);
} elseif( $do == "search" ) {
	if( isset($_REQUEST['mode']) AND $_REQUEST['mode'] == "advanced" ) {
		$canonical = dle_ml_public_url('search_advanced', array(), $config);
	} else {
		$canonical = dle_ml_public_url('search', array(), $config);
	}
}
...
if (dle_ml_is_utility_request($do, isset($doaction) ? $doaction : '', isset($_REQUEST['action']) ? $_REQUEST['action'] : '')) {
...
$hreflang_links = dle_ml_hreflang_links();
if (is_array($hreflang_links) AND count($hreflang_links)) {
	foreach ($hreflang_links as $href_lang) {
		if (!isset($href_lang['hreflang']) OR !isset($href_lang['href'])) continue;
		$meta->push('link', [
			'rel' => 'alternate',
			'hreflang' => $href_lang['hreflang'],
			'href' => $href_lang['href']
		]);
	}
}
