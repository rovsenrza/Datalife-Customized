...
HTML;
	...
	$ml_languages = dle_ml_ordered_languages(dle_ml_main_folder($config));
	$ml_title_inputs = array();
	$ml_short_inputs = array();

	foreach ($ml_languages as $folder => $meta) {
		$title_label = htmlspecialchars($meta['title'], ENT_QUOTES, 'UTF-8');
		$title_value = htmlspecialchars(dle_ml_get_setting_value('home_title', $folder, $config['home_title']), ENT_QUOTES, 'UTF-8');
		$short_value = htmlspecialchars(dle_ml_get_setting_value('short_title', $folder, $config['short_title']), ENT_QUOTES, 'UTF-8');

		$ml_title_inputs[] = "<label class=\"display-block mb-5\">{$title_label}</label><input dir=\"auto\" type=\"text\" class=\"form-control mb-10\" name=\"save_con_ml[home_title][{$folder}]\" value=\"{$title_value}\">";
		$ml_short_inputs[] = "<label class=\"display-block mb-5\">{$title_label}</label><input dir=\"auto\" type=\"text\" class=\"form-control mb-10\" name=\"save_con_ml[short_title][{$folder}]\" value=\"{$short_value}\">";
	}
	...
	showRow( "Site Title By Language", "Frontend site title values for each language", implode("", $ml_title_inputs) );
	...
	showRow( "Short Title By Language", "Frontend short title values for each language", implode("", $ml_short_inputs) );
	...
	showRow( "Multilanguage URL Prefix", "If enabled, non-active frontend languages use URL prefixes (/az, /en, /ru). Active language stays prefixless.", makeCheckBox( "save_con[multilanguage_prefix]", isset($config['multilanguage_prefix']) ? "{$config['multilanguage_prefix']}" : "1" ) );
...
HTML;
	...
	$save_con_ml = isset($_POST['save_con_ml']) && is_array($_POST['save_con_ml']) ? $_POST['save_con_ml'] : array();
	$available_ml = dle_ml_get_languages();

	if( !isset($save_con['main_language']) OR !isset($available_ml[$save_con['main_language']]) ) {
		$save_con['main_language'] = dle_ml_main_folder($config);
	}
	...
	$save_con['multilanguage_prefix'] = isset($save_con['multilanguage_prefix']) ? intval($save_con['multilanguage_prefix']) : 0;
	...
	if( isset($save_con_ml['home_title'][$save_con['main_language']]) ) {
		$save_con['home_title'] = trim((string)$save_con_ml['home_title'][$save_con['main_language']]);
	}

	if( isset($save_con_ml['short_title'][$save_con['main_language']]) ) {
		$save_con['short_title'] = trim((string)$save_con_ml['short_title'][$save_con['main_language']]);
	}
	...
	$ml_home_title_values = array();
	$ml_short_title_values = array();

	foreach ($available_ml as $folder => $meta) {
		$ml_home_title_values[$folder] = isset($save_con_ml['home_title'][$folder]) ? trim((string)$save_con_ml['home_title'][$folder]) : '';
		$ml_short_title_values[$folder] = isset($save_con_ml['short_title'][$folder]) ? trim((string)$save_con_ml['short_title'][$folder]) : '';
	}

	dle_ml_set_setting_translations('home_title', $ml_home_title_values);
	dle_ml_set_setting_translations('short_title', $ml_short_title_values);
