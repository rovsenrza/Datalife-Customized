...
if( $action == "list" ) {
	...
	$_SESSION['admin_referrer'] = isset($_SESSION['admin_referrer']) ? htmlspecialchars($_SERVER['REQUEST_URI'], ENT_QUOTES, 'UTF-8' ) : '';
...
HTML;
	...
	if( isset($ml_translations_data[$ml_main_folder]) ) {
		foreach( array('title', 'short_story', 'full_story', 'descr', 'keywords', 'metatitle', 'tags') as $ml_field ) {
			if( isset($ml_translations_data[$ml_main_folder][$ml_field]) AND trim((string)$ml_translations_data[$ml_main_folder][$ml_field]) !== '' ) {
...
HTML;
	...
	if( is_array($ml_translations_data) AND count($ml_translations_data) ) {
		foreach($ml_translations_data as $ml_folder => $ml_item) {
			if( !is_array($ml_item) ) $ml_item = array();
			$ml_translations_data[$ml_folder]['title'] = isset($ml_item['title']) ? $parse->decodeBBCodes($ml_item['title'], false) : '';
			$ml_translations_data[$ml_folder]['short_story'] = isset($ml_item['short_story']) ? $parse->decodeBBCodes($ml_item['short_story'], true, true) : '';
			$ml_translations_data[$ml_folder]['full_story'] = isset($ml_item['full_story']) ? $parse->decodeBBCodes($ml_item['full_story'], true, true) : '';
			$ml_translations_data[$ml_folder]['descr'] = isset($ml_item['descr']) ? $parse->decodeBBCodes($ml_item['descr'], false) : '';
...
HTML;
	...
	function auto_keywords_lang(langFolder, key) {
		...
		$.post(\"engine/ajax/controller.php?mod=keywords\", { short_txt: short_txt, full_txt: full_txt, key: key, user_hash: '{$dle_login_hash}' }, function(data){
			...
			if (key == 1) {
				descrField.val(data);
			} else if (keyField.length) {
				keyField.tokenfield('setTokens', data);
			}
...
HTML;
	...
	foreach ($ml_extra_languages as $ml_folder => $ml_meta) {
		...
		$ml_panes_html .= <<<HTML
<div class="tab-pane" id="tabml_{$ml_code_safe}">
	<div class="form-group">
	  <label class="control-label col-sm-2">{$lang['edit_et']}</label>
	  <div class="col-sm-10">
		<input type="text" dir="auto" class="form-control width-550" name="translations[{$ml_folder_safe}][title]" value="{$ml_value_title}" maxlength="250">
	  </div>
	 </div>
	 <div class="form-group editor-group">
	  <label class="control-label col-md-2">{$lang['addnews_short']}</label>
	  <div class="col-md-10">
		<div class="editor-panel"><textarea id="ml_short_story_{$ml_code_safe}" name="translations[{$ml_folder_safe}][short_story]" class="wysiwygeditor" style="width:98%;height:300px;">{$ml_value_short}</textarea></div>
	  </div>
	 </div>
	 <div class="form-group editor-group">
	  <label class="control-label col-md-2">{$lang['addnews_full']}</label>
	  <div class="col-md-10">
		<div class="editor-panel"><textarea id="ml_full_story_{$ml_code_safe}" name="translations[{$ml_folder_safe}][full_story]" class="wysiwygeditor" style="width:98%;height:300px;">{$ml_value_full}</textarea></div>
	  </div>
	 </div>
	 <div class="form-group">
	  <label class="control-label col-md-2 col-sm-3">{$lang['addnews_tags']}</label>
	  <div class="col-md-10 col-sm-9">
		<input type="text" dir="auto" name="translations[{$ml_folder_safe}][tags]" class="form-control width-500" value="{$ml_value_tags}">
	  </div>
	 </div>
	 <div class="form-group">
	  <label class="control-label col-md-2 col-sm-3">{$lang['meta_title']}</label>
	  <div class="col-md-10 col-sm-9">
		<input type="text" dir="auto" name="translations[{$ml_folder_safe}][metatitle]" class="form-control width-500" maxlength="140" value="{$ml_value_metatitle}">
	  </div>
	 </div>
	 <div class="form-group">
	  <label class="control-label col-md-2 col-sm-3">{$lang['meta_descr']}</label>
	  <div class="col-md-10 col-sm-9">
		<input type="text" dir="auto" name="translations[{$ml_folder_safe}][descr]" id="ml_autodescr_{$ml_code_safe}" class="form-control width-500" maxlength="300" value="{$ml_value_descr}">
	  </div>
	 </div>
	 <div class="form-group">
	  <label class="control-label col-md-2 col-sm-3">{$lang['meta_keys']}</label>
	  <div class="col-md-10 col-sm-9">
		<textarea dir="auto" class="tags ml-keywords" name="translations[{$ml_folder_safe}][keywords]" id="ml_keywords_{$ml_code_safe}">{$ml_value_keywords}</textarea><br><br>
		<input type="button" onclick="auto_keywords_lang('{$ml_folder_safe}',1); return false;" class="btn bg-primary-600 btn-sm btn-raised position-left" value="{$lang['btn_descr']}">
...
HTML;
	...
	$full_story_parsed = $parse->BB_Parse( $full_story );
	$short_story_parsed = $parse->BB_Parse( $short_story );

	$full_story = $db->safesql( $full_story_parsed );
	$short_story = $db->safesql( $short_story_parsed );
		...
		if( $have_perm ) {
			...
			if( $ifdelete != "yes" ) {
				...
				dle_ml_save_post_translation($item_db[0], $ml_main_folder, array(
					'title' => $title_main,
					'short_story' => $short_story_parsed,
					'full_story' => $full_story_parsed,
					'descr' => stripslashes($metatags['description']),
					'keywords' => stripslashes($metatags['keywords']),
				...
				foreach ($ml_languages as $ml_folder => $ml_meta) {
					if ($ml_folder == $ml_main_folder || !isset($ml_translations[$ml_folder]) || !is_array($ml_translations[$ml_folder])) continue;

					$ml_item = $ml_translations[$ml_folder];
					$ml_title = $parse->process(trim(strip_tags(isset($ml_item['title']) ? (string)$ml_item['title'] : '')));
					$ml_short = $parse->BB_Parse($parse->process(isset($ml_item['short_story']) ? (string)$ml_item['short_story'] : ''));
					$ml_full = $parse->BB_Parse($parse->process(isset($ml_item['full_story']) ? (string)$ml_item['full_story'] : ''));
					$ml_descr = trim(isset($ml_item['descr']) ? (string)$ml_item['descr'] : '');
					$ml_keywords = trim(isset($ml_item['keywords']) ? (string)$ml_item['keywords'] : '');
					$ml_metatitle = trim(isset($ml_item['metatitle']) ? (string)$ml_item['metatitle'] : '');
					$ml_tags = isset($ml_item['tags']) ? (string)$ml_item['tags'] : '';
					if( preg_match( "/[\||\<|\>]/", $ml_tags ) ) $ml_tags = "";
					else $ml_tags = htmlspecialchars( strip_tags( stripslashes( trim( $ml_tags ) ) ), ENT_COMPAT, 'UTF-8' );

					if (!$ml_descr || !$ml_keywords || !$ml_metatitle) {
						$ml_meta_source = dle_strlen($ml_full) > 12 ? $ml_full : $ml_short;
						$_REQUEST['meta_title'] = $ml_metatitle;
						$_REQUEST['descr'] = $ml_descr;
						$_REQUEST['keywords'] = $ml_keywords;
						$ml_meta = create_metatags($ml_meta_source, true);
						if (!$ml_descr) $ml_descr = stripslashes($ml_meta['description']);
						if (!$ml_keywords) $ml_keywords = stripslashes($ml_meta['keywords']);
						if (!$ml_metatitle) $ml_metatitle = stripslashes($ml_meta['title']);
					}

					dle_ml_save_post_translation($item_db[0], $ml_folder, array(
						'title' => $ml_title,
						'short_story' => $ml_short,
						'full_story' => $ml_full,
						'descr' => $ml_descr,
