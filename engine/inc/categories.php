...
if( $action == "add" ) {
	...
	dle_ml_save_category_translation($new_category_id, $ml_main_folder, array(
		'name' => $cat_name_raw,
		'descr' => stripslashes($description),
		'keywords' => stripslashes($keywords),
		'metatitle' => stripslashes($meta_title),
		'fulldescr' => stripslashes($fulldescr),
	));

	if( isset($_POST['cat_name_i18n']) AND is_array($_POST['cat_name_i18n']) ) {
		foreach($_POST['cat_name_i18n'] as $ml_folder => $ml_name) {
			if( !isset($ml_languages[$ml_folder]) OR $ml_folder == $ml_main_folder ) continue;
			$ml_name = htmlspecialchars(strip_tags(stripslashes($ml_name)), ENT_QUOTES, 'UTF-8');
			dle_ml_save_category_translation($new_category_id, $ml_folder, array('name' => $ml_name));
		}
	}
...
$(function(){
	...
	$ml_main_folder = dle_ml_main_folder($config);
	$ml_languages = dle_ml_ordered_languages($ml_main_folder);
	$ml_extra_languages = $ml_languages;
	unset($ml_extra_languages[$ml_main_folder]);
	$ml_translations_data = dle_ml_get_category_translations($catid);

	if( isset($ml_translations_data[$ml_main_folder]['name']) AND trim((string)$ml_translations_data[$ml_main_folder]['name']) !== '' ) {
		$row['name'] = $ml_translations_data[$ml_main_folder]['name'];
	}
	...
	foreach($ml_extra_languages as $ml_folder => $ml_meta) {
		...
		$ml_name = stripslashes( preg_replace( array ("'\"'", "'\''" ), array ("&quot;", "&#039;" ), $ml_name ) );
		$ml_folder_safe = htmlspecialchars($ml_folder, ENT_QUOTES, 'UTF-8');
		$ml_title = htmlspecialchars($ml_meta['title'], ENT_QUOTES, 'UTF-8');
		$ml_name_inputs_html .= "<div class=\"form-group\"><label class=\"control-label col-md-2 col-sm-3\">{$lang['cat_name']} ({$ml_title})</label><div class=\"col-md-10 col-sm-9\"><input class=\"form-control width-350\" value=\"{$ml_name}\" maxlength=\"50\" type=\"text\" dir=\"auto\" name=\"cat_name_i18n[{$ml_folder_safe}]\"></div></div>";
	}
...
HTML;
	...
	if( isset($_POST['cat_name_i18n']) AND is_array($_POST['cat_name_i18n']) ) {
		foreach($_POST['cat_name_i18n'] as $ml_folder => $ml_name) {
			if( !isset($ml_languages[$ml_folder]) OR $ml_folder == $ml_main_folder ) continue;
			$ml_name = htmlspecialchars(strip_tags(stripslashes($ml_name)), ENT_QUOTES, 'UTF-8');
			dle_ml_save_category_translation($catid, $ml_folder, array('name' => $ml_name));
		}
	}
...
$ml_main_folder = dle_ml_main_folder($config);
$ml_languages = dle_ml_ordered_languages($ml_main_folder);
$ml_modal_name_inputs = '';

foreach($ml_languages as $ml_folder => $ml_meta) {
	if( $ml_folder == $ml_main_folder ) continue;
	$ml_folder_safe = htmlspecialchars($ml_folder, ENT_QUOTES, 'UTF-8');
	$ml_title = htmlspecialchars($ml_meta['title'], ENT_QUOTES, 'UTF-8');
	$ml_modal_name_inputs .= "<div class=\"form-group\"><div class=\"row\"><div class=\"col-sm-12\"><label>{$lang['cat_name']} ({$ml_title})</label><input name=\"cat_name_i18n[{$ml_folder_safe}]\" type=\"text\" dir=\"auto\" class=\"form-control\" maxlength=\"50\"></div></div></div>";
}
