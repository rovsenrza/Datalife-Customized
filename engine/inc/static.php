...
HTML;
	...
	$ml_languages = dle_ml_admin_languages($config, true);
	$ml_extra_languages = $ml_languages;
	unset($ml_extra_languages[$ml_main_folder]);
	$ml_codes = array();

	foreach ($ml_languages as $ml_folder => $ml_meta) {
		$ml_codes[$ml_folder] = isset($ml_meta['code']) ? $ml_meta['code'] : dle_ml_code_from_folder_name($ml_folder);
	}
	$ml_codes_json = json_encode($ml_codes, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
	...
	var mlLangCodes = {$ml_codes_json};

	function auto_keywords_lang_static(langFolder, key) {
		tinyMCE.triggerSave();

		var langCode = mlLangCodes[langFolder] ? mlLangCodes[langFolder] : langFolder.toLowerCase().substring(0, 2);
		var shortField = document.getElementById('ml_template_' + langCode);
		var descrField = $('#ml_autodescr_' + langCode);
		var keyField = $('#ml_keywords_' + langCode);

		var short_txt = shortField ? shortField.value : '';
		...
		$.post(\"engine/ajax/controller.php?mod=keywords\", { short_txt: short_txt, key: key, user_hash: '{$dle_login_hash}' }, function(data){
			...
			if (key == 1) { descrField.val(data); }
			else if (keyField.length) { keyField.tokenfield('setTokens', data); }
	...
	$ml_static_add_nav_tabs_html = '';
	$ml_static_add_panes_html = '';
	$ml_static_add_heading_html = $lang['static_a'];
	$ml_static_add_body_open = '<div class="panel-body">';
	$ml_static_add_body_close = '</div>';

	if (count($ml_extra_languages)) {

		foreach($ml_extra_languages as $ml_folder => $ml_meta) {
			$ml_code = isset($ml_codes[$ml_folder]) ? $ml_codes[$ml_folder] : dle_ml_code_from_folder_name($ml_folder);
			$ml_title = htmlspecialchars($ml_meta['title'], ENT_QUOTES, 'UTF-8');
			$ml_folder_safe = htmlspecialchars($ml_folder, ENT_QUOTES, 'UTF-8');
			$ml_code_safe = htmlspecialchars($ml_code, ENT_QUOTES, 'UTF-8');

			$ml_static_add_nav_tabs_html .= "<li><a href=\"#tabml_static_{$ml_code_safe}\" data-toggle=\"tab\">{$ml_title}</a></li>";

			$ml_static_add_panes_html .= <<<HTML
<div class="tab-pane" id="tabml_static_{$ml_code_safe}">
	<div class="panel-body">
		<div class="form-group">
		  <label class="control-label col-md-2">{$lang['static_descr']}</label>
		  <div class="col-md-10">
			<input type="text" dir="auto" name="translations[{$ml_folder_safe}][descr]" class="form-control width-550" maxlength="250">
		  </div>
		 </div>
		<div class="form-group editor-group">
		  <label class="control-label col-md-2">{$lang['static_templ']}</label>
		  <div class="col-md-10">
			<div class="editor-panel"><textarea name="translations[{$ml_folder_safe}][template]" id="ml_template_{$ml_code_safe}" class="wysiwygeditor" style="width:98%;height:300px;"></textarea></div>
		  </div>
		 </div>
		<div class="form-group">
		  <label class="control-label col-md-2">{$lang['meta_title']}</label>
		  <div class="col-md-10">
			<input type="text" name="translations[{$ml_folder_safe}][metatitle]" class="form-control width-500" maxlength="140">
		  </div>
		 </div>
		<div class="form-group">
		  <label class="control-label col-md-2">{$lang['meta_descr']}</label>
		  <div class="col-md-10">
			<input type="text" dir="auto" name="translations[{$ml_folder_safe}][metadescr]" id="ml_autodescr_{$ml_code_safe}" class="form-control width-500" maxlength="300">
		  </div>
		 </div>
		<div class="form-group">
		  <label class="control-label col-md-2">{$lang['meta_keys']}</label>
		  <div class="col-md-10">
			<textarea dir="auto" class="tags ml-static-keywords" name="translations[{$ml_folder_safe}][metakeys]" id="ml_keywords_{$ml_code_safe}"></textarea><br><br>
			<button onclick="auto_keywords_lang_static('{$ml_folder_safe}', 1); return false;" class="btn bg-primary-600 btn-sm btn-raised position-left"><i class="fa fa-exchange position-left"></i>{$lang['btn_descr']}</button>
			<button onclick="auto_keywords_lang_static('{$ml_folder_safe}', 2); return false;" class="btn bg-primary-600 btn-sm btn-raised"><i class="fa fa-exchange position-left"></i>{$lang['btn_keyword']}</button>
		  </div>
		 </div>
	</div>
</div>
HTML;
		...
		$ml_static_add_heading_html = <<<HTML
		<ul class="nav nav-tabs nav-tabs-solid">
			<li class="active"><a href="#tabhome_static" data-toggle="tab"><i class="fa fa-file-text-o position-left"></i>{$lang['static_a']}</a></li>
			{$ml_static_add_nav_tabs_html}
		</ul>
HTML;
		$ml_static_add_body_open = '<div class="panel-tab-content tab-content"><div class="tab-pane active" id="tabhome_static"><div class="panel-body">';
		$ml_static_add_body_close = "</div></div>{$ml_static_add_panes_html}</div>";
	}
...
HTML;
	...
	$descr_raw = htmlspecialchars( strip_tags(trim($_POST['description'])), ENT_QUOTES, 'UTF-8' );
	$descr = $db->safesql( $descr_raw );
	...
	dle_ml_save_static_translation($row, $ml_main_folder, array(
		'descr' => $descr_raw,
		'template' => $template_main,
		'metadescr' => stripslashes($metatags['description']),
		'metakeys' => stripslashes($metatags['keywords']),
	...
	$ml_req_keywords = isset($_REQUEST['keywords']) ? $_REQUEST['keywords'] : '';

	foreach($ml_languages as $ml_folder => $ml_meta) {
		if( $ml_folder == $ml_main_folder OR !isset($ml_translations[$ml_folder]) OR !is_array($ml_translations[$ml_folder]) ) continue;

		$ml_item = $ml_translations[$ml_folder];
		$ml_descr = htmlspecialchars( strip_tags(trim(isset($ml_item['descr']) ? (string)$ml_item['descr'] : '')), ENT_QUOTES, 'UTF-8' );
		$ml_template = isset($ml_item['template']) ? (string)$ml_item['template'] : '';
		$ml_metatitle = trim(isset($ml_item['metatitle']) ? (string)$ml_item['metatitle'] : '');
		$ml_metadescr = trim(isset($ml_item['metadescr']) ? (string)$ml_item['metadescr'] : '');
		$ml_metakeys = trim(isset($ml_item['metakeys']) ? (string)$ml_item['metakeys'] : '');

		if (!$ml_metatitle || !$ml_metadescr || !$ml_metakeys) {
			$_REQUEST['meta_title'] = $ml_metatitle;
			$_REQUEST['descr'] = $ml_metadescr;
			$_REQUEST['keywords'] = $ml_metakeys;
			$ml_meta_tags = create_metatags($ml_template, true);
			if (!$ml_metatitle) $ml_metatitle = stripslashes($ml_meta_tags['title']);
			if (!$ml_metadescr) $ml_metadescr = stripslashes($ml_meta_tags['description']);
			if (!$ml_metakeys) $ml_metakeys = stripslashes($ml_meta_tags['keywords']);
		}

		dle_ml_save_static_translation($row, $ml_folder, array(
			'descr' => $ml_descr,
			'template' => $ml_template,
			'metadescr' => $ml_metadescr,
			'metakeys' => $ml_metakeys,
			'metatitle' => $ml_metatitle,
		));
	}

	$_REQUEST['meta_title'] = $ml_req_meta_title;
	$_REQUEST['descr'] = $ml_req_descr;
	$_REQUEST['keywords'] = $ml_req_keywords;
	...
	$ml_codes_json = json_encode($ml_codes, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
	$ml_translations_data = isset($row['id']) ? dle_ml_get_static_translations($row['id']) : array();

	if( isset($ml_translations_data[$ml_main_folder]) ) {
		foreach(array('descr' => 'descr', 'template' => 'template', 'metadescr' => 'metadescr', 'metakeys' => 'metakeys', 'metatitle' => 'metatitle') as $ml_src => $ml_target) {
			if( isset($ml_translations_data[$ml_main_folder][$ml_src]) AND trim((string)$ml_translations_data[$ml_main_folder][$ml_src]) !== '' ) {
				$row[$ml_target] = $ml_translations_data[$ml_main_folder][$ml_src];
			}
		}
	}

	foreach($ml_translations_data as $ml_folder => $ml_item) {
		if( !is_array($ml_item) ) $ml_item = array();
		$ml_translations_data[$ml_folder]['descr'] = isset($ml_item['descr']) ? stripslashes($ml_item['descr']) : '';
		$ml_translations_data[$ml_folder]['template'] = isset($ml_item['template']) ? $parse->decodeBBCodes($ml_item['template'], true, true) : '';
		$ml_translations_data[$ml_folder]['metadescr'] = isset($ml_item['metadescr']) ? stripslashes($ml_item['metadescr']) : '';
		$ml_translations_data[$ml_folder]['metakeys'] = isset($ml_item['metakeys']) ? stripslashes($ml_item['metakeys']) : '';
...
HTML;
	...
	$ml_static_edit_nav_tabs_html = '';
	$ml_static_edit_panes_html = '';
	$ml_static_edit_heading_html = $lang['static_edit'];
	$ml_static_edit_body_open = '<div class="panel-body">';
	$ml_static_edit_body_close = '</div>';
	...
	if( count($ml_extra_languages) ) {
		...
		foreach($ml_extra_languages as $ml_folder => $ml_meta) {
			...
			$ml_descr = isset($ml_values['descr']) ? htmlspecialchars($ml_values['descr'], ENT_QUOTES, 'UTF-8') : '';
			$ml_template = isset($ml_values['template']) ? htmlspecialchars($ml_values['template'], ENT_QUOTES, 'UTF-8') : '';
			$ml_metatitle = isset($ml_values['metatitle']) ? htmlspecialchars($ml_values['metatitle'], ENT_QUOTES, 'UTF-8') : '';
			$ml_metadescr = isset($ml_values['metadescr']) ? htmlspecialchars($ml_values['metadescr'], ENT_QUOTES, 'UTF-8') : '';
			$ml_metakeys = isset($ml_values['metakeys']) ? htmlspecialchars($ml_values['metakeys'], ENT_QUOTES, 'UTF-8') : '';

			$ml_static_edit_nav_tabs_html .= "<li><a href=\"#tabml_static_{$ml_code_safe}\" data-toggle=\"tab\">{$ml_title}</a></li>";

			$ml_static_edit_panes_html .= <<<HTML
<div class="tab-pane" id="tabml_static_{$ml_code_safe}">
	<div class="panel-body">
		<div class="form-group">
		  <label class="control-label col-md-2">{$lang['static_descr']}</label>
		  <div class="col-md-10">
			<input type="text" dir="auto" name="translations[{$ml_folder_safe}][descr]" class="form-control width-550" maxlength="250" value="{$ml_descr}">
		  </div>
		 </div>
		<div class="form-group editor-group">
		  <label class="control-label col-md-2">{$lang['static_templ']}</label>
		  <div class="col-md-10">
			<div class="editor-panel"><textarea name="translations[{$ml_folder_safe}][template]" id="ml_template_{$ml_code_safe}" class="wysiwygeditor" style="width:98%;height:300px;">{$ml_template}</textarea></div>
		  </div>
		 </div>
		<div class="form-group">
		  <label class="control-label col-md-2">{$lang['meta_title']}</label>
		  <div class="col-md-10">
			<input type="text" name="translations[{$ml_folder_safe}][metatitle]" class="form-control width-500" maxlength="140" value="{$ml_metatitle}">
		  </div>
		 </div>
		<div class="form-group">
		  <label class="control-label col-md-2">{$lang['meta_descr']}</label>
		  <div class="col-md-10">
			<input type="text" dir="auto" name="translations[{$ml_folder_safe}][metadescr]" id="ml_autodescr_{$ml_code_safe}" class="form-control width-500" maxlength="300" value="{$ml_metadescr}">
		  </div>
		 </div>
		<div class="form-group">
		  <label class="control-label col-md-2">{$lang['meta_keys']}</label>
		  <div class="col-md-10">
			<textarea dir="auto" class="tags ml-static-keywords" name="translations[{$ml_folder_safe}][metakeys]" id="ml_keywords_{$ml_code_safe}">{$ml_metakeys}</textarea><br><br>
			<button onclick="auto_keywords_lang_static('{$ml_folder_safe}', 1); return false;" class="btn bg-primary-600 btn-sm btn-raised position-left"><i class="fa fa-exchange position-left"></i>{$lang['btn_descr']}</button>&nbsp;
			<button onclick="auto_keywords_lang_static('{$ml_folder_safe}', 2); return false;" class="btn bg-primary-600 btn-sm btn-raised"><i class="fa fa-exchange position-left"></i>{$lang['btn_keyword']}</button>
		  </div>
		 </div>
...
HTML;
		...
		$ml_static_edit_heading_html = <<<HTML
		<ul class="nav nav-tabs nav-tabs-solid">
			<li class="active"><a href="#tabhome_static" data-toggle="tab"><i class="fa fa-file-text-o position-left"></i>{$lang['static_edit']}</a></li>
			{$ml_static_edit_nav_tabs_html}
		</ul>
HTML;
		$ml_static_edit_body_open = '<div class="panel-tab-content tab-content"><div class="tab-pane active" id="tabhome_static"><div class="panel-body">';
		$ml_static_edit_body_close = "</div></div>{$ml_static_edit_panes_html}</div>";
	}
...
HTML;
	...
	$descr_raw = htmlspecialchars( $_POST['description'], ENT_QUOTES, 'UTF-8' );
	$descr = trim( $db->safesql( $descr_raw ) );
	$disable_index = isset( $_POST['disable_index'] ) ? intval( $_POST['disable_index'] ) : 0;
	$disable_search = isset( $_POST['disable_search'] ) ? intval( $_POST['disable_search'] ) : 0;
	...
	if( isset($_GET['page']) AND $_GET['page'] == "rules" ) {
		...
		if( !$static_ml_id ) {
			$ml_row = $db->super_query( "SELECT id FROM " . PREFIX . "_static WHERE name='dle-rules-page'" );
			$static_ml_id = isset($ml_row['id']) ? intval($ml_row['id']) : 0;
		}

		if( $static_ml_id ) {
			dle_ml_save_static_translation($static_ml_id, $ml_main_folder, array(
				'descr' => $descr_raw,
				'template' => $template_main,
				'metadescr' => stripslashes($metatags['description']),
				'metakeys' => stripslashes($metatags['keywords']),
			...
			foreach($ml_languages as $ml_folder => $ml_meta) {
				...
				dle_ml_save_static_translation($static_ml_id, $ml_folder, array(
					'descr' => $ml_descr,
					'template' => $ml_template,
					'metadescr' => $ml_metadescr,
					'metakeys' => $ml_metakeys,
		...
		dle_ml_save_static_translation($id, $ml_main_folder, array(
			'descr' => $descr_raw,
			'template' => $template_main,
			'metadescr' => stripslashes($metatags['description']),
			'metakeys' => stripslashes($metatags['keywords']),
		...
		foreach($ml_languages as $ml_folder => $ml_meta) {
			...
			dle_ml_save_static_translation($id, $ml_folder, array(
				'descr' => $ml_descr,
				'template' => $ml_template,
				'metadescr' => $ml_metadescr,
				'metakeys' => $ml_metakeys,
