...
if( !defined('DATALIFEENGINE') ) {
	...
	if ( isset($row['id']) AND $row['id'] AND $perm ) {
		...
		$news_page_int = intval($news_page);
		$cstart_int = intval($cstart);

		if( $config['allow_read_count'] AND ($news_page_int < 2) AND ($cstart_int < 2) ) {
		...
		if( $config['related_news'] AND $view_template != "print") {
			...
			if( $related_buffer === false ) {
				...
				if ( $row['related_ids'] ) {
					...
					$related_query_id = $db->query( "SELECT id, date, short_story, xfields, title, category, alt_name FROM " . PREFIX . "_post WHERE id IN({$row['related_ids']}) AND approve=1 ORDER BY FIND_IN_SET(id, '{$row['related_ids']}') LIMIT " . $config['related_number'], false );
					$first_show = false;
					...
					$related_query_id = $db->query( "SELECT id, date, short_story, xfields, title, category, alt_name, MATCH (title, short_story, full_story, xfields) AGAINST ('{$body}') as score FROM " . PREFIX . "_post {$join_category}WHERE {$where_category}{$allowed_cats}{$not_allowed_cats}MATCH (title, short_story, full_story, xfields) AGAINST ('{$body}') AND id != " . $row['id'] . " AND approve=1" . $where_date . " ORDER BY score DESC LIMIT " . $config['related_number'], false );
