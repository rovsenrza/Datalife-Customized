	...
	foreach ( $temp as $key => $value ) {
		...
		if( $buffer === false ) {
			...
			foreach ( $xml->content as $content ) {
				...
				if( $value['tmax'] and dle_strlen( $content['title'] ) > $value['tmax'] ) $content['title'] = dle_substr( $content['title'], 0, $value['tmax'] ) . " ...";
				...
				if( $value['dmax'] and dle_strlen( $content['description'] ) > $value['dmax'] ) {
					
					$content['description'] = dle_substr( $content['description'], 0, $value['dmax'] );
					
					if( ($temp_dmax = dle_strrpos( $content['description'], ' ' )) ) $content['description'] = dle_substr( $content['description'], 0, $temp_dmax );
					
					$content['description'] .= " ...";
				
				}

				$content['link'] = htmlspecialchars ($content['link'], ENT_QUOTES, 'UTF-8');
