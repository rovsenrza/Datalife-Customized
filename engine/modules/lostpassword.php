...
$canonical = false;
if( (!isset($_GET['action']) OR !$_GET['action']) AND (!isset($_GET['douser']) OR !intval($_GET['douser'])) ) {
...
if( $is_logged ) {
	...
	$lostpassword_form_action = dle_ml_public_url('lostpassword', array(), $config);
	$tpl->copy_template = "<form  method=\"post\" name=\"dle-lostpassword\" id=\"dle-lostpassword\" action=\"{$lostpassword_form_action}\">\n" . $tpl->copy_template . "<input name=\"submit_lost\" type=\"hidden\" id=\"submit_lost\" value=\"submit_lost\" /></form>";
